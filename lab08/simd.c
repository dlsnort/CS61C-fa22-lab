...
long long int sum_simd(int vals[NUM_ELEMS]) {
    ...
    for(unsigned int w = 0; w < OUTER_ITERATIONS; w++) {
        ...
        int tmp_arr[4];
        for (unsigned int i = 0; i < NUM_ELEMS / 4 * 4; i += 4) {
            __m128i raw = _mm_loadu_si128((__m128i_u *)(vals + i));
            __m128i mask = _mm_cmpgt_epi32(raw, _127);
            __m128i tmp = _mm_and_si128(raw, mask);
            sum_vec = _mm_add_epi32(tmp, sum_vec);
        }
        _mm_store_si128((__m128i *) tmp_arr, sum_vec);
...
long long int sum_simd_unrolled(int vals[NUM_ELEMS]) {
    ...
    for(unsigned int w = 0; w < OUTER_ITERATIONS; w++) {
        ...
        for (unsigned int i = 0; i < NUM_ELEMS / 16 * 16; i += 16) {
            ...
            raw = _mm_loadu_si128((__m128i_u *)(vals + i + 4));
            mask = _mm_cmpgt_epi32(raw, _127);
            tmp = _mm_and_si128(raw, mask);
            sum_vec = _mm_add_epi32(tmp, sum_vec);

            raw = _mm_loadu_si128((__m128i_u *)(vals + i + 8));
            mask = _mm_cmpgt_epi32(raw, _127);
            tmp = _mm_and_si128(raw, mask);
            sum_vec = _mm_add_epi32(tmp, sum_vec);

            raw = _mm_loadu_si128((__m128i_u *)(vals + i + 12));
            mask = _mm_cmpgt_epi32(raw, _127);
            tmp = _mm_and_si128(raw, mask);
            sum_vec = _mm_add_epi32(tmp, sum_vec);
        }
